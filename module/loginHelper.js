// [FIX] Đã sửa lỗi forced_account_switch
'use strict';
const fs = require('fs');
...
function setJarFromPairs(j, pairs, domain) {
  ...
  for (const kv of pairs) {
    // [FIX] Bỏ qua cookie i_user để tránh lỗi "Chuyển tài khoản"
    if (kv.startsWith('i_user=')) {
      continue;
    }
...
function cookieHeaderFromJar(j) {
  ...
  for (const u of parts) {
    ...
    for (const kv of s.split(';')) {
      ...
      // [FIX] Không xuất i_user ra header
      if (name === 'i_user') continue;
      seen.add(name);
      parts.push(t);
    }
  }
...
async function ensureUniqueIndex(sequelize) {
  ...
  try {
    await sequelize
      .getQueryInterface()
      .addIndex('app_state_backups', ['userID', 'type'], { unique: true, name: 'app_state_user_type_unique' });
  } catch {}
...
const MESSENGER_USER_AGENT =
  'Dalvik/2.1.0 (Linux; U; Android 9; ASUS_Z01QD Build/PQ3A.190605.003) [FBAN/Orca-Android;FBAV/391.2.0.20.404;FBPN/com.facebook.orca;FBLC/vi_VN;FBBV/437533963;FBCR/Viettel Telecom;FBMF/asus;FBBD/asus;FBDV/ASUS_Z01QD;FBSV/9;FBCA/x86:armeabi-v7a;FBDM/{density=1.5,width=1600,height=900};FB_FW/1;]';
...
async function setJarCookies(j, appstate) {
  ...
  for (const c of appstate) {
    // [FIX] Không set cookie i_user
    if (c.key === 'i_user') continue;
...
async function loginViaGraph(username, password, twofactorSecretOrCode, i_user, externalJar) {
  ...
  const client = wrapper(
    axiosBase.create({ jar: cookieJar, withCredentials: true, timeout: 30000, validateStatus: () => true }),
  );
  ...
  const res1 = await client.request({
    ...
    method: 'post',
    data: qs.stringify(form1),
    headers,
  });
  if (res1.status === 200 && res1.data && res1.data.session_cookies) {
    const appstate = res1.data.session_cookies.map((c) => ({
      key: c.name,
      value: c.value,
      domain: c.domain,
      path: c.path,
    ...
    // [FIX] Xóa logic tự động thêm i_user
    ...
    try {
      const r1 = await client.request({
        url: `https://api.facebook.com/method/auth.getSessionforApp?format=json&access_token=${res1.data.access_token}&new_app_id=350685531728`,
        method: 'get',
        headers: {
          'user-agent': MESSENGER_USER_AGENT,
          'x-fb-connection-type': 'WIFI',
          authorization: `OAuth ${res1.data.access_token}`,
        },
      });
    ...
    try {
      const r2 = await client.request({
        url: `https://api.facebook.com/method/auth.getSessionforApp?format=json&access_token=${res1.data.access_token}&new_app_id=275254692598279`,
        method: 'get',
        headers: {
          'user-agent': MESSENGER_USER_AGENT,
          'x-fb-connection-type': 'WIFI',
    ...
    return {
      ok: true,
      cookies: appstate.map((c) => ({ key: c.key, value: c.value })),
      jar: cookieJar,
      access_token_mess: res1.data.access_token || null,
      access_token: eaau,
      access_token_eaad6v7: eaad6v7,
      uid: res1.data.uid || cUserCookie?.value || null,
      session_key: res1.data.session_key || null,
    };
  }
  ...
  if (err && err.code === 406) {
    ...
    const res2 = await client.request({
      url: 'https://b-graph.facebook.com/auth/login',
      method: 'post',
      data: qs.stringify(form2),
      headers,
    });
    if (res2.status === 200 && res2.data && res2.data.session_cookies) {
      const appstate = res2.data.session_cookies.map((c) => ({
        key: c.name,
        value: c.value,
        domain: c.domain,
        path: c.path,
      ...
      try {
        const r1 = await client.request({
          url: `https://api.facebook.com/method/auth.getSessionforApp?format=json&access_token=${res2.data.access_token}&new_app_id=350685531728`,
          method: 'get',
          headers: {
            'user-agent': MESSENGER_USER_AGENT,
            'x-fb-connection-type': 'WIFI',
      ...
      try {
        const r2 = await client.request({
          url: `https://api.facebook.com/method/auth.getSessionforApp?format=json&access_token=${res2.data.access_token}&new_app_id=275254692598279`,
          method: 'get',
          headers: {
            'user-agent': MESSENGER_USER_AGENT,
            'x-fb-connection-type': 'WIFI',
      ...
      return {
        ...
        cookies: appstate.map((c) => ({ key: c.key, value: c.value })),
        jar: cookieJar,
        access_token_mess: res2.data.access_token || null,
        access_token: eaau,
        access_token_eaad6v7: eaad6v7,
        uid: res2.data.uid || cUserCookie?.value || null,
        session_key: res2.data.session_key || null,
      };
    }
...
async function tryAutoLoginIfNeeded(currentHtml, currentCookies, globalOptions, ctxRef) {
  // [FIX] Chỉ ưu tiên c_user để lấy UID
  const getUID = (cs) => cs.find((c) => c.key === 'c_user')?.value || cs.find((c) => c.name === 'c_user')?.value;
    ...
    (async () => {
      ...
      ctx.bypassAutomation = async function (resp, j) {
        ...
        const u = (r) =>
          ...
          (r?.config?.baseURL ? new URL(r.config.url || '/', r.config.baseURL).toString() : r?.config?.url || '');
        ...
        const cookieUID = async () => {
          try {
            ...
            // [FIX] Chỉ lấy c_user
            return cookies.find((c) => c.key === 'c_user')?.value;
          } catch {
            return undefined;
          }
        };
        ...
        const bypass = async (body) => {
          ...
          const fb_dtsg =
            getFrom(b, '"DTSGInitData",[],{"token":"', '",') || b.match(/name="fb_dtsg"\s+value="([^"]+)"/)?.[1];
          const jazoest =
      ...
      .then(async function (res) {
        ...
        // [FIX] Chỉ lấy userID từ c_user
        let userID = cookies.find((c) => c.key === 'c_user')?.value || cookies.find((c) => c.name === 'c_user')?.value;
